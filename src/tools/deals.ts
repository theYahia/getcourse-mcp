...
export const getDealsSchema = z.object({
  status: z.enum([
    "new", "in_work", "payment_waiting",
    "finished", "cancelled", "false_deal",
  ]).optional().describe("Фильтр по статусу заказа"),
  created_from: z.string().optional().describe("Дата создания от (YYYY-MM-DD)"),
  created_to: z.string().optional().describe("Дата создания до (YYYY-MM-DD)"),
});
...
export async function handleGetDeals(params: z.infer<typeof getDealsSchema>): Promise<string> {
  ...
  const exportResp = (await client.get("/account/deals", {
  ...
  if (!exportResp.success && exportResp.info?.export_id) {
    ...
    return "Экспорт заказов занимает слишком долго. Попробуйте позже.";
  }
